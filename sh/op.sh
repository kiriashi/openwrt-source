...
rm -rf package/mtk/drivers/mt_wifi/patches-7672/001-eeprom-flash-api.patch

rm -rf package/mtk/drivers/mt_wifi/patches-7672/040-fix-rrm-beacon-rep-zero-length-array-fortify-warning.patch

rm -rf package/mtk/drivers/mt_wifi/patches-7672/050-eeprom-read-from-device-tree.patch
...
CI_VERSION=20250408
CI_SOURCE=mt79xx_conninfra_20250408-f2fa25.tar.xz
CI_HASH=3a82cfa69e68c8e584723b604dda565b2c085239ba1ac834d6d3b0ebd987e58c
CI_SOURCE_URL=https://raw.githubusercontent.com/immortalwrt-mt798x/immortalwrt/refs/heads/openwrt-21.02/dl

sed -i \
  -e "s|^\(PKG_VERSION[[:space:]]*:=[[:space:]]*\).*|\1$CI_VERSION|" \
  -e "s|^\(PKG_SOURCE[[:space:]]*:=[[:space:]]*\).*|\1$CI_SOURCE|" \
  -e "s|^\(PKG_HASH[[:space:]]*:=[[:space:]]*\).*|\1$CI_HASH|" \
  -e "s|^\(PKG_SOURCE_URL[[:space:]]*:=[[:space:]]*\).*|\1$CI_SOURCE_URL|" \
